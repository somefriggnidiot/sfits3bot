...
public class commandUnsuspend extends ChatCommand{

	commandUnsuspend(String commandName, boolean isEnabled, CommandAccessLevel requiredAccessLevel) {
	...
	int handle(ClientInfo invoker, CommandAccessLevel invokerAccessLevel, String input) {
		if (CommandAccessLevel.getAccessLevel(invokerAccessLevel) >= CommandAccessLevel.getAccessLevel(requiredAccessLevel)) {
			...
			if (matchingClients.size() > 1) {
				...
				System.out.println(Main.timeStamp() + "Unsuspending " + match.getNickname() + ".");
				api.removeClientFromServerGroup(Config.getSuspendedGroupId(), api.getDatabaseClientByUId(match.getUniqueIdentifier()).getDatabaseId());
				return 0;
			}
